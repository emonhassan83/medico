...
const UpdateProfile = () => {
  ...
  const { data, isLoading } = useGetSinglePatientQuery(profile?.id);
    ...
    <>
      ...
      <div className="mx-4 flex items-center justify-between mt-4">
        ...
        <div className="flex items-center gap-1 text-[#495057] text-sm">
          ...
          <Link href="/patient/profile-view">Patient Profile</Link>
        </div>
      </div>

      <div className="mt-5 ml-4">
        <Link
          href="/patient/profile-view"
          className="text-white text-sm bg-[#556ee6] py-2 px-4 rounded-md"
        >
          <ArrowLeftOutlined className="mr-1" /> Back to Profile
        </Link>
      </div>
